...
def recomButton(contentJson, badEvaluations):
	contentBase= BeautifulSoup(contentJson, "lxml") 
	
	images = contentBase.find_all('button')
	...
	messages[1] = gettext ("Se recomienda que agregue un título al botón, utilizando el atributo 'title'.")
...
class Petition(Resource):
	def post(self):
		...
		try: 
			...
			response = recomButton(json_data['html'], json_data['badResults']) 
			return response
		except requests.exceptions.RequestException as e: 
			return ("error")

api.add_resource(Petition, '/button_recomendation')
...
if __name__ == '__main__':
	app.run( debug = False, host='0.0.0.0', port=9016)