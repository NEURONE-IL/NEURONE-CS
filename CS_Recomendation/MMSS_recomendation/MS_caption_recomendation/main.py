...
def recomCaption(contentJson, badEvaluations):
  contentBase= BeautifulSoup(contentJson, "lxml") 
  
  images = contentBase.find_all('caption')
...
class Petition(Resource):
  def post(self):
    ...
    try: 
      ...
      response = recomCaption(json_data['html'], json_data['badResults']) 
      return response
    except requests.exceptions.RequestException as e: 
      return ("error")


api.add_resource(Petition, '/caption_recomendation')
...
if __name__ == '__main__':
	app.run( debug = False, host='0.0.0.0', port=9017)