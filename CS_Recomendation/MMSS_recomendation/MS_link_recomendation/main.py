...
def recomLink(contentJson, badEvaluations):
    contentBase= BeautifulSoup(contentJson, "lxml") 
    
    images = contentBase.find_all('link')
    ...
    messages[1] = gettext ("Se recomienda no utilizar el atributo 'media' definido como 'aural' o 'speech'.")
...
class Petition(Resource):
    def post(self):
        ...
        try:
            ...
            response = recomLink(json_data['html'], json_data['badResults'])
            return response
        except requests.exceptions.RequestException as e:
            return ("error")

api.add_resource(Petition, '/link_recomendation')
...
if __name__ == '__main__':
	app.run( debug = False, host='0.0.0.0', port=9030)