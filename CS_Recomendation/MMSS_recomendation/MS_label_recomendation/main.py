...
def recomLabel(contentJson, badEvaluations):
	contentBase= BeautifulSoup(contentJson, "lxml") 
	
	images = contentBase.find_all('label')
	...
	messages[1] = gettext ("Se debe idicar un texto dentro de la etiqueta 'label' asociada a la etiqueta 'input'.")
...
class Petition(Resource):
	def post(self):
	    ...
	    try: 
	      ...
	      response = recomLabel(json_data['html'], json_data['badResults']) 
	      return response
	    except requests.exceptions.RequestException as e: 
	      return ("error")

api.add_resource(Petition, '/label_recomendation')
...
if __name__ == '__main__':
	app.run( debug = False, host='0.0.0.0', port=9028)