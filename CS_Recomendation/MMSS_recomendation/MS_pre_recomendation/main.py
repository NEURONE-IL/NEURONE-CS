...
def recomPre(contentJson, badEvaluations):
	contentBase= BeautifulSoup(contentJson, "lxml") 
	
	images = contentBase.find_all('pre')
	...
	messages[1] = gettext ("Se recomienda que no utilice espacios para simular una tabla dentro de la etiqueta 'pre'.")
...
class Petition(Resource):
	def post(self):
		...
		try:
			...
			response = recomPre(json_data['html'], json_data['badResults'])
			return response
		except requests.exceptions.RequestException as e:
			return ("error")

api.add_resource(Petition, '/pre_recomendation')
...
if __name__ == '__main__':
	app.run( debug = False, host='0.0.0.0', port=9033)