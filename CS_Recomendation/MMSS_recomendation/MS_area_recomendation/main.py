...
def recomArea(contentJson, badEvaluations):
	contentBase= BeautifulSoup(contentJson, "lxml") 
	
	images = contentBase.find_all('area')
	...
	messages[1] = gettext ("Se recomienda definir un texto alternativo, utilizando el atributo 'alt'.")
...
class Petition(Resource):
	def post(self):
		...
		try: 
			...
			response = recomArea(json_data['html'], json_data['badResults']) 
			return response
		except requests.exceptions.RequestException as e: 
			return ("error")

api.add_resource(Petition, '/area_recomendation')
...
if __name__ == '__main__':
	app.run( debug = False, host='0.0.0.0', port=9014)