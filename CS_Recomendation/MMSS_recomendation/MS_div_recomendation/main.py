...
def recomDiv(contentJson, badEvaluations):
	contentBase= BeautifulSoup(contentJson, "lxml") 
	
	images = contentBase.find_all('div')
	...
	messages[1] = gettext ("Se recomienda no utilizar las etiquetas 'div' anidadas, si utilizan el atributo 'aria-label'.")
...
class Petition(Resource):
	def post(self):
	    ...
	    try: 
	      ...
	      response = recomDiv(json_data['html'], json_data['badResults']) 
	      return response
	    except requests.exceptions.RequestException as e: 
	      return ("error")

api.add_resource(Petition, '/div_recomendation')
...
if __name__ == '__main__':
	app.run( debug = False, host='0.0.0.0', port=9019)