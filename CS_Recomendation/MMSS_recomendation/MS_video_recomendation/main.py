...
def recomVideo(contentJson, badEvaluations):
    contentBase= BeautifulSoup(contentJson, "lxml")
    
    images = contentBase.find_all('video')
    ...
    messages[1] = gettext ("Se recomienda que no utilice los atributos de 'title', 'aria-label' o 'aria-labelledby' ni contenido de apoyo (descripción).")
...
class Petition(Resource):
    def post(self):
        ...
        try:
            ...
            response = recomVideo(json_data['html'], json_data['badResults'])
            return response
        except requests.exceptions.RequestException as e:
            return ("error")

api.add_resource(Petition, '/video_recomendation')
...
if __name__ == '__main__':
	app.run( debug = False, host='0.0.0.0', port=9036)