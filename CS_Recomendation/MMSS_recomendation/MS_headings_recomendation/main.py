...
def recomHeadings(contentJson, badEvaluations):
    contentBase= BeautifulSoup(contentJson, "lxml") 
    images = []
    for tag in ['h1','h2','h3','h4','h5','h6']:
        images += (contentBase.find_all(tag))
    ...
    messages[1] = gettext ("Se recomienda no anidar las etiquetas h1, h2, h3, h4, h5 ó h6, debe existir algun otro elemento entre títulos.")
    #Tipo de incompatibilidad 2:
    messages[2] = gettext ("Se recomienda no utilizar las etiquetas h1, h2, h3, h4, h5 ó h6 si solo contienen signos de puntuación.")
...
class Petition(Resource):
    def post(self):
        ...
        try: 
          ...
          response = recomHeadings(json_data['html'], json_data['badResults']) 
          return response
        except requests.exceptions.RequestException as e: 
          return ("error")

api.add_resource(Petition, '/headings_recomendation')
...
if __name__ == '__main__':
    app.run( debug = False, host='0.0.0.0', port=9024)