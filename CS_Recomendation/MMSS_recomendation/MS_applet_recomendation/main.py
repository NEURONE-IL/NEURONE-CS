...
def recomApplet(contentJson, badEvaluations):
	contentBase= BeautifulSoup(contentJson, "lxml") 
	
	images = contentBase.find_all('applet')
	...
	messages[1] = gettext ("Se recomienda utilizar una descripción y no utilizar los atributos 'alt', 'title' ni 'aria-labelledby'.")
...
class Petition(Resource):
	def post(self):
		...
		try: 
			...
			response = recomApplet(json_data['html'], json_data['badResults']) 
			return response
		except requests.exceptions.RequestException as e: 
			return ("error")

api.add_resource(Petition, '/applet_recomendation')
