...
def recomDd(contentJson, badEvaluations):
	contentBase= BeautifulSoup(contentJson, "lxml") 
	
	images = contentBase.find_all('dd')
	...
	messages[1] = gettext ("Se recomienda que no utilizar imágenes dentro de etiqueta 'dd'.")
...
class Petition(Resource):
	def post(self):
	    ...
	    try: 
	      ...
	      response = recomDd(json_data['html'], json_data['badResults']) 
	      return response
	    except requests.exceptions.RequestException as e: 
	      return ("error")

api.add_resource(Petition, '/dd_recomendation')
...
if __name__ == '__main__':
	app.run( debug = False, host='0.0.0.0', port=9018)