...
def recomInput(contentJson, badEvaluations):
    contentBase= BeautifulSoup(contentJson, "lxml") 
    
    images = contentBase.find_all('input')
    ...
    messages[1] = gettext ("Se debe agregar una descripción utilizando un el atributo 'alt'.")
...
class Petition(Resource):
    def post(self):
        ...
        try:
            ...
            response = recomInput(json_data['html'], json_data['badResults'])
            return response
        except requests.exceptions.RequestException as e:
            return ("error")

api.add_resource(Petition, '/input_recomendation')
...
if __name__ == '__main__':
	app.run( debug = False, host='0.0.0.0', port=9027)