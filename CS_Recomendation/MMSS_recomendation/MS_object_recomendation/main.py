...
def recomObject(contentJson, badEvaluations):
	contentBase= BeautifulSoup(contentJson, "lxml") 
	
	images = contentBase.find_all('object')
	...
	messages[1] = gettext ("Se recomienda que la etiqueta 'object' tenga descripción y que no utilice los atributos 'aria-label', 'aria-labelledby', 'alt' ni 'title'.")
...
class Petition(Resource):
	def post(self):
		...
		try: 
			...
			response = recomObject(json_data['html'], json_data['badResults'])
			return response
		except requests.exceptions.RequestException as e:
			return ("error")

api.add_resource(Petition, '/object_recomendation')
...
if __name__ == '__main__':
	app.run( debug = False, host='0.0.0.0', port=9032)