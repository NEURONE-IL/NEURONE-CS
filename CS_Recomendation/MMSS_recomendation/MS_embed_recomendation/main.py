...
def recomEmbed(contentJson, badEvaluations):
	contentBase= BeautifulSoup(contentJson, "lxml") 

	images = contentBase.find_all('embed')
	...
	messages[1] = gettext ("Se recomienda agregar una descripción y no utilizar los atributos 'aria-labelledby', 'alt' y 'title'.")
...
class Petition(Resource):
	def post(self):
	    ...
	    try: 
	      ...
	      response = recomEmbed(json_data['html'], json_data['badResults']) 
	      return response
	    except requests.exceptions.RequestException as e: 
	      return ("error")

api.add_resource(Petition, '/embed_recomendation')
...
if __name__ == '__main__':
	app.run( debug = False, host='0.0.0.0', port=9020)