...
def recomAudio(contentJson, badEvaluations):
	contentBase= BeautifulSoup(contentJson, "lxml") 
	
	images = contentBase.find_all('audio')
...
class Petition(Resource):
	def post(self):
		...
		try: 
			...
			response = recomAudio(json_data['html'], json_data['badResults']) 
			return response
		except requests.exceptions.RequestException as e: 
			return ("error")

api.add_resource(Petition, '/audio_recomendation')
...
if __name__ == '__main__':
	app.run( debug = False, host='0.0.0.0', port=9015)