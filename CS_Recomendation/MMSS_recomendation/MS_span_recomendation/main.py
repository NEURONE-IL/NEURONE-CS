...
def recomSpan(contentJson, badEvaluations):
	contentBase= BeautifulSoup(contentJson, "lxml") 

	images = contentBase.find_all('span')
	...
	messages[1] = gettext ("Se recomienda no utilizar espacios para separar las letras de las palabras.")
...
class Petition(Resource):
	def post(self):
		...
		try: 
			...
			response = recomSpan(json_data['html'], json_data['badResults']) 
			return response
		except requests.exceptions.RequestException as e: 
			return ("error")

api.add_resource(Petition, '/span_recomendation')
...
if __name__ == '__main__':
	app.run( debug = False, host='0.0.0.0', port=9034)