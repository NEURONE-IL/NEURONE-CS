...
def recomTable(contentJson, badEvaluations):
    contentBase= BeautifulSoup(contentJson, "lxml") 

    images = contentBase.find_all('table')
    ...
    messages[1] = gettext ("Se le recomienda agregar dentro del la etiqueta 'table' las etiquetas 'tfoot' y th' ó 'thead'.")
    #Tipo de incompatibilidad 2:
    messages[2] = gettext ("Se recomienda no utilizar los atributos 'role', 'presentation' ni 'note'.")
    #Tipo de incompatibilidad 3:
    messages[3] = gettext ("Se recomienda agregar la etiqueta 'th' ó utilizar el atributo 'columnheader' en las etiquetas 'td'.")
...
class Petition(Resource):
    def post(self):
        ...
        try:
            ...
            response = recomTable(json_data['html'], json_data['badResults'])
            return response
        except requests.exceptions.RequestException as e:
            return ("error")

api.add_resource(Petition, '/table_recomendation')
