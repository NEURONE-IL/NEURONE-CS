...
Space separated tables - 100%
    ...
    -w: f34 / w: f33 -a / f34 -a / f48 -a
'''

def evaluatePre(contentJson):
	contentBase= BeautifulSoup(contentJson, "lxml")

	pres = contentBase.find_all('pre')
	totalQuantity = len(pres)
	...
	for pre in pres:
		...
		prePlaceholder = pre.text
		spacesPlaceholder = prePlaceholder.count("  ")
		if (spacesPlaceholder>1):
			flag = 1
			typeError = 1
		else:
			good += 1
		...
		try:
			...
			for key in json_data:
				if (key == 'html'):
					...
					response = evaluatePre(json_data[key])
			return response
		except requests.exceptions.RequestException as e:
			return ("error")

api.add_resource(Petition, '/pre_evaluation')
