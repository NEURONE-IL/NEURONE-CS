...
'''Definition lists con images as bullets - 100%
    <dl>
       <dd><img src='img16x16.png' alt='* '>Audrey</dd>
       <dd><img src='img16x16.png' alt='* '>Laurie</dd>
       <dd><img src='img16x16.png' alt='* '>Alice</dd>
    </dl>
    -//
'''

def evaluateDd(contentJson):
	contentBase= BeautifulSoup(contentJson, "lxml")

	dds = contentBase.find_all('dd')
	totalQuantity = len(dds)
	...
	for dd in dds:
	    numId += 1
	    parentName = dd.parent.name
	    nextAux = dd.findNext()
	    nextImg = BeautifulSoup(str(nextAux), 'lxml').find_all('img')

		#Definition lists con images as bullets - 100%
	    if(parentName == 'dl' and nextImg):
	    	flag = 1
	    	typeError = 1
		...
		try:
			...
			for key in json_data:
				if (key == 'html'):
					...
					response = evaluateDd(json_data[key])
			return response
		except requests.exceptions.RequestException as e:
			return ("error")

api.add_resource(Petition, '/dd_evaluation')
