...
def evaluateObject(contentJson):
	contentBase= BeautifulSoup(contentJson, "lxml")

	objects = contentBase.find_all('object')
	totalQuantity = len(objects)
	...
	for objec in objects:
		numId += 1
		objectAttrAriaLabBy = objec.get('aria-labelledby')
		objectAttrAriaLab = objec.get('aria-label')
		objectAttrTitle = objec.get('title')
		objectAttrAlt = objec.get('alt')
		objectPlaceholder = objec.text

		if (objectPlaceholder):
			objectPlaceholder = re.sub(r"[ \t\n\x0B\f\r]", '', objectPlaceholder, flags=0)


		#OBJECT con ARIA-LABEL attribute - 100%
		#OBJECT con ARIA-LABELLEDBY attribute - 100%
		#OBJECT con ALT attribute - 100%
		#OBJECT con TITLE attribute - 100%
		#OBJECT sin description - 100%
		if(objectAttrAriaLab or objectAttrAriaLabBy or objectAttrAlt or objectAttrTitle or not objectPlaceholder):
			flag = 1
			typeError = 1

		#OBJECT con fallback content - 75%
		if(objectPlaceholder):
			good += 1
		...
		if (flag == 1):
			failed += 1
			pos = position(contentBase, objec)
			pos.append(numId)
		...
		try:
			...
			for key in json_data:
				if (key == 'html'):
					...
					response = evaluateObject(json_data[key])
			return response
		except requests.exceptions.RequestException as e:
			return ("error")

api.add_resource(Petition, '/object_evaluation')
...
if __name__ == '__main__':
	app.run( debug = False, host='0.0.0.0', port=8032)
