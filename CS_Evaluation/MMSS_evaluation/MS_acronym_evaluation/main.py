...
'''
si <acronym> tiene titulo - 100%
	<acronym title='Three Letter Acronym'>TLN</acronym>
	-//
'''

def evaluateAcronym(contentJson):
	contentBase= BeautifulSoup(contentJson, "lxml")

	acronyms = contentBase.find_all('acronym')
	totalQuantity = len(acronyms)
	...
	for acronym in acronyms:
		...
		if (flag == 1):
			failed += 1
			pos = position(contentBase, acronym)
			pos.append(numId)
		...
		try:
			...
			for key in json_data:
				if (key == 'html'):
					...
					response = evaluateAcronym(json_data[key])
			return response
		except requests.exceptions.RequestException as e:
			return ("error")

api.add_resource(Petition, '/acronym_evaluation')
